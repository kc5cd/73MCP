...
def discover_ports() -> list[str]:
    ...
    "found but not opened" step of AntScopeZ's own device search.
    """
    matches = []
...
class NanovnaDevice:
    ...
    def read_until_prompt(self) -> list[str]:
        ...
        Mirrors AntScopeZ's own parser: every command's reply ends this
        way, and no fixed line count is ever assumed. A read timeout
        (empty line from read_line()) also ends the loop defensively, so
        a device that stops responding mid-reply doesn't hang the caller
        forever.
        """
        lines: list[str] = []
        while True:
