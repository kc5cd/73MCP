...
Ported from this repo's own C++ reference implementation
(analyzer/nanovna_analyzer.cpp) -- see README.md's "Protocol reference"
section for the command sequence and citations. Only the classic ASCII
sweep/frequencies/data path is implemented here (proven working against
real hardware); the newer binary "scan" fast path is not needed for v1
and is left for a future addition.
"""
...
# USB identification (analyzer/nanovna_analyzer.h).
USB_VID = 0x0483
USB_PID = 0x5740
...
# as the sole end-of-reply marker, mirroring this repo's own C++ parser
...
def parse_re_im(line: str) -> complex:
    ...
    permissive, a malformed line yields 0+0j rather than raising,
    matching the reference implementation's own tolerance for the
    occasional short/garbage line.
    """
    tok = line.split()
    if len(tok) < 2:
        return complex(0, 0)
...
def impedance_from_reflection(gamma: complex, z0: float = Z0) -> complex:
    ...
    (analyzer/nanovna_analyzer.cpp) ported to Python -- that function
    itself was added to deduplicate two copies of this same formula in
