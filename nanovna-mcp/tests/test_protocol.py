...
Note: the confirmed-working example cited in README.md (sweep
420-540MHz, SWR ~3.0/1.05/2.8, captured live 2026-08-30) was recorded as
derived SWR values only; the raw re/im lines from that session were
never saved, so it can't be used as a byte-exact fixture here. Instead
these tests check the impedance/SWR math against well-known analytic
reference points (open, short, matched load) that are independently
verifiable, plus the line parsers against both well-formed and
malformed input.
"""
...
def test_swr_known_magnitude():
    ...
    # magnitude as the real 420MHz-edge SWR (~3.0:1) cited in README.md,
